...
drawTriangle(x1, y1, x2, y2, x3, y3) {
    ...
    if (!this.isV(this.r - 1, this.c) && !this.isV(this.r, this.c - 1)) {
        points.push({x: x + size / 2, y: y});
        points.push({x: x, y: y + size / 2});
    } else { points.push({x: x, y: y}); }

    if (!this.isV(this.r + 1, this.c) && !this.isV(this.r, this.c - 1)) {
        points.push({x: x, y: y + size / 2});
        points.push({x: x + size / 2, y: y + size});
    } else { points.push({x: x, y: y + size}); }

    if (!this.isV(this.r + 1, this.c) && !this.isV(this.r, this.c + 1)) {
        points.push({x: x + size / 2, y: y + size});
        points.push({x: x + size, y: y + size / 2});
    } else { points.push({x: x + size, y: y + size}); }

    if (!this.isV(this.r - 1, this.c) && !this.isV(this.r, this.c + 1)) {
        points.push({x: x + size, y: y + size / 2});
        points.push({x: x + size / 2, y: y});
    } else { points.push({x: x + size, y: y}); }
...
ctx.stroke();
    ...
    if (!this.isV(this.r-1, this.c) && !this.isV(this.r, this.c-1)) { // Top-Left
        ctx.moveTo(x + size/2, y); ctx.lineTo(x, y + size/2);
    }
    if (!this.isV(this.r+1, this.c) && !this.isV(this.r, this.c-1)) { // Bottom-Left
        ctx.moveTo(x, y + size/2); ctx.lineTo(x + size/2, y + size);
    }
    if (!this.isV(this.r+1, this.c) && !this.isV(this.r, this.c+1)) { // Bottom-Right
        ctx.moveTo(x + size/2, y + size); ctx.lineTo(x + size, y + size/2);
    }
    if (!this.isV(this.r-1, this.c) && !this.isV(this.r, this.c+1)) { // Top-Right
        ctx.moveTo(x + size, y + size/2); ctx.lineTo(x + size/2, y);
    }
    ctx.stroke();
    ...
    if (this.walls[2] && this.isV(this.r + 1, this.c)) {
        ctx.moveTo(x + size, y + size); ctx.lineTo(x, y + size);
    }
